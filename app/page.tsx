...
export default function DoublesMatchupApp() {
  ...
  const getMatchForCourt = (currentCourts: Court[], currentMembers: Member[]) => {
    ...
    if (config.levelStrict) {
      ...
      // 各レベルごとに「次に試合に出るべき人」をソートして保持
      const levelGroups = levels.map(l => {
        const membersInLevel = candidates.filter(m => m.level === l);
        const sorted = membersInLevel.sort((a, b) => {
          if (a.playCount !== b.playCount) return a.playCount - b.playCount;
          return a.lastPlayedTime - b.lastPlayedTime;
        });
        return { level: l, members: sorted };
      });

      // 4人以上いるレベルの中から、最も「平均試合数が少ない」レベルを選択
      const validGroups = levelGroups
        .filter(g => g.members.length >= 4)
        .sort((a, b) => {
          const avgA = a.members.slice(0, 4).reduce((sum, m) => sum + m.playCount, 0) / 4;
          const avgB = b.members.slice(0, 4).reduce((sum, m) => sum + m.playCount, 0) / 4;
          return avgA - avgB;
        });

      if (validGroups.length > 0) {
        bestSelection = validGroups[0].members.slice(0, 4);
      }
    }

    // レベル厳密でない、または厳密モードで4人揃わなかった場合
    if (bestSelection.length < 4) {
      if (config.levelStrict) return null; // 厳密モード時は他レベルを混ぜない

      // ユニット（固定ペア優先）での選出ロジック
      type Unit = { members: Member[], avgPlay: number, lastTime: number };
      const units: Unit[] = [];
      ...
      candidates.forEach(m => {
        ...
        if (m.fixedPairMemberId && candidates.some(c => c.id === m.fixedPairMemberId)) {
          ...
          units.push({ members: [m, partner], avgPlay: (m.playCount + partner.playCount) / 2, lastTime: Math.max(m.lastPlayedTime, partner.lastPlayedTime) });
        } else {
          processedIds.add(m.id);
          units.push({ members: [m], avgPlay: m.playCount, lastTime: m.lastPlayedTime });
        }
      });

      const sortedUnits = units.sort((a, b) => a.avgPlay - b.avgPlay || a.lastTime - b.lastTime || Math.random() - 0.5);
  ...
  const generateNextMatch = (courtId: number) => {
    ...
    if (!match) return alert('待機メンバーが足りません（レベル厳密モードの場合、同じレベルが4名必要です）');
  ...
  const changeZoom = (delta: number) => {
    setConfig(prev => ({ ...prev, zoomLevel: Math.max(0.5, Math.min(2.0, prev.zoomLevel + delta)) }));
  };
                    ...
                    <div className="flex items-center gap-2 h-full">
                      <div className="flex-1 grid grid-cols-2 gap-2 h-full">
                        <div className="bg-blue-50 rounded-lg flex flex-col justify-center items-center border border-blue-100 px-2 overflow-hidden py-1">
                          <div className="w-full text-center leading-tight mb-1 font-black text-blue-900 whitespace-nowrap overflow-hidden text-ellipsis" style={{ fontSize: getDynamicFontSize(members.find(m => m.id === court.match?.p1)?.name) }}>{members.find(m => m.id === court.match?.p1)?.name}</div>
                          <div className="w-full text-center leading-tight font-black text-blue-900 whitespace-nowrap overflow-hidden text-ellipsis" style={{ fontSize: getDynamicFontSize(members.find(m => m.id === court.match?.p2)?.name) }}>{members.find(m => m.id === court.match?.p2)?.name}</div>
                        </div>
                        <div className="bg-red-50 rounded-lg flex flex-col justify-center items-center border border-red-100 px-2 overflow-hidden py-1">
                          <div className="w-full text-center leading-tight mb-1 font-black text-red-900 whitespace-nowrap overflow-hidden text-ellipsis" style={{ fontSize: getDynamicFontSize(members.find(m => m.id === court.match?.p3)?.name) }}>{members.find(m => m.id === court.match?.p3)?.name}</div>
                          <div className="w-full text-center leading-tight font-black text-red-900 whitespace-nowrap overflow-hidden text-ellipsis" style={{ fontSize: getDynamicFontSize(members.find(m => m.id === court.match?.p4)?.name) }}>{members.find(m => m.id === court.match?.p4)?.name}</div>
                        </div>
                      </div>
                ...
                <div key={m.id} className={`p-4 flex items-center gap-4 ${!m.isActive ? 'bg-gray-50 opacity-40' : ''}`}>
                  <div className="flex-1">
                    ...
                    <div className="flex items-center gap-3 mt-1 flex-wrap">
                      <button onClick={() => handleLevelChange(m.id)} className={`text-xs font-bold rounded-md px-3 py-1 text-white transition-colors ${m.level === 'A' ? 'bg-blue-600' : m.level === 'B' ? 'bg-yellow-500' : 'bg-red-500'}`}>レベル{m.level}</button>
                ...
                <div className="fixed inset-0 bg-black/50 z-50 flex items-center justify-center p-4" onClick={() => setEditingPairMemberId(null)}>
                  <div className="bg-white rounded-xl shadow-xl w-full max-w-sm overflow-hidden" onClick={e => e.stopPropagation()}>
                    ...
                    <div className="max-h-[60vh] overflow-y-auto p-2">
                      <button onClick={() => updateFixedPair(editingPairMemberId, null)} className="w-full text-left px-4 py-3 hover:bg-red-50 text-red-600 font-bold border-b border-gray-100 flex items-center gap-2"><Unlink size={16} /> ペアを解消</button>
                      {members.filter(m => m.id !== editingPairMemberId && m.isActive && (!m.fixedPairMemberId || m.fixedPairMemberId === editingPairMemberId) && m.level === members.find(x => x.id === editingPairMemberId)?.level).map(candidate => (
                        <button key={candidate.id} onClick={() => updateFixedPair(editingPairMemberId, candidate.id)} className={`w-full text-left px-4 py-3 hover:bg-blue-50 font-bold border-b border-gray-100 flex items-center gap-2 ${members.find(x => x.id === editingPairMemberId)?.fixedPairMemberId === candidate.id ? 'bg-blue-50 text-blue-700' : 'text-gray-700'}`}>
                          <LinkIcon size={16} className="text-gray-400" />{candidate.name}<span className="text-xs text-gray-400 ml-auto font-normal">Level {candidate.level}</span>
                        </button>
                      ))}
                    </div>
                  </div>
              ...
              <tbody className="divide-y divide-gray-100">
                {matchHistory.map(h => (
                  <tr key={h.id}>
                    ...
                    <td className="p-4 font-bold text-base">{getLevelBadge(h.level)} {h.players[0]}, {h.players[1]} <span className="text-gray-300 px-1 font-normal italic">vs</span> {h.players[2]}, {h.players[3]}</td>
                  </tr>
                ))}
              </tbody>
            </table>
        ...
        {activeTab === 'settings' && (
          <div className="bg-white rounded-2xl shadow-sm p-8 space-y-8 max-w-2xl mx-auto">
            ...
            <div className="space-y-4">
              <button onClick={resetPlayCountsOnly} className="w-full py-4 bg-gray-50 text-gray-700 rounded-2xl font-bold flex items-center justify-center gap-3 border border-gray-200 active:bg-gray-100 transition-colors"><RotateCcw size={20} /> 試合数と履歴をリセット</button>
              <button onClick={() => {if(confirm('名簿を含め全てリセットしますか？')) {localStorage.clear(); location.reload();}}} className="w-full py-4 bg-red-50 text-red-500 rounded-2xl font-bold border border-red-100 active:bg-red-100 transition-colors">データを完全消去</button>
            </div>
          </div>
        )}
      </main>
